...
const client = createCounterClient({
  endpoint: import.meta.env.DEV || location.hostname.endsWith('.netlify.app') ? '/api/game-count' : 'https://balloon-fight-zyra.netlify.app/api/game-count',
...
if (!import.meta.env.DEV || localApi) {
  ...
  const refresh = () => { if (!document.hidden && (!document.getElementById('home').hidden)) void client.refresh(); };
