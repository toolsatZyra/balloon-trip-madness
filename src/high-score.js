...
let selectedMode = 'trip', records = {}, trace;
...
const client = createScoreClient({
  endpoint: import.meta.env.DEV || location.hostname.endsWith('.netlify.app') ? '/api/high-score' : 'https://balloon-fight-zyra.netlify.app/api/high-score',
...
if (enabled) {
  refreshHighScore();
  const refresh = () => { if (!document.hidden && (!document.getElementById('home').hidden)) refreshHighScore(); };
  setInterval(refresh, 5000);
  document.addEventListener('visibilitychange', refresh);
} else { records = { trip: 120 }; render(); }
